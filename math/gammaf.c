...
#ifdef __libnix__
#define polevl __polevl
static __inline__ float polevl( float xx, float *coef, int N )
{
float ans, x=xx, *p=coef;
int i;
...
#define p1evl __p1evl
static __inline__ float p1evl( float xx, float *coef, int N )
{
float ans, x=xx, *p=coef;
int i;
 ...
 */
#ifdef ANSIC
static __inline__ float stirf( float xx )
#else
...
ALIAS(lgammaf,gamma);
ALIAS(lgammal,gamma);
ALIAS(lgamma_r,gamma_r);
ALIAS(lgammaf_r,gamma_r);
ALIAS(lgammal_r,gamma_r);

#ifdef ANSIC
float gamma_r( float xx, int *signp )
#else
float gamma_r(xx,signp)
double xx;
int *signp;
#endif
{
...
MFALIAS(float,gamma)(float xx)
{ return gamma_r(xx,&sgngamf); }
